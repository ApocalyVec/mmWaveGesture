...
if __name__ == '__main__':
    ...
    mc = ModelCheckpoint(
        'F:/indexPen/trained_models/bestSoFar_indexPen_CRNN' + str(datetime.datetime.now()).replace(':', '-').replace(' ',
                                                                                                                      '_') + '.h5',
    ...
    history = model.fit_generator(generator=training_gen, validation_data=validation_gen, use_multiprocessing=True, workers=8, shuffle=True, epochs=epochs, callbacks=[es, mc])
